...
function sceneResultPage(character, outcome) {
  const name = escapeHtml(character?.name ?? '無名之人');
  const historyCount = Array.isArray(character?.actionHistory) ? character.actionHistory.length : 0;
  return shell(`<div class="game-workspace"><main class="hero play-scene"><p class="eyebrow">IMMORTALVOYAGE</p><p class="scene-name">${name}</p><h1>此行有果</h1><p class="subtitle">${escapeHtml(outcome.result)}</p><a class="button" href="/play">回到眼前</a><p class="notice">此行已寫入角色存檔，目前共保存 ${historyCount} 筆情境行動。</p></main><aside class="ad-slot" aria-label="廣告"></aside></div>`);
}
...
export function starterWorkPage(character) {
  ...
  if (active) {
    return shell(`<div class="game-workspace"><main class="hero work-page"><p class="eyebrow">IMMORTALVOYAGE</p><p class="scene-name">${name}</p><h1>今日有工</h1><div class="work-card active-work"><h2>${escapeHtml(active.title)}</h2><p class="work-pay">報酬 ${active.pay} 銅</p><dl><div><dt>伙食</dt><dd>${escapeHtml(provisionLabel(active.provisions?.food))}</dd></div><div><dt>飲水</dt><dd>${escapeHtml(provisionLabel(active.provisions?.water))}</dd></div><div><dt>住宿</dt><dd>${escapeHtml(provisionLabel(active.provisions?.lodging))}</dd></div></dl><form method="post" action="/work/complete"><button class="button" type="submit">完成這份工作</button></form></div><p class="notice">目前持有 ${copper} 銅；完成後由伺服器驗證並結算報酬。</p><a class="text-link" href="/play">返回眼前</a></main><aside class="ad-slot" aria-label="廣告"></aside></div>`);
  }
  const offers = getStarterWorkOffers(character);
  return shell(`<div class="game-workspace"><main class="hero work-page"><p class="eyebrow">IMMORTALVOYAGE</p><p class="scene-name">${name}</p><h1>找份活做</h1><p class="subtitle compact">能做的事不多，但至少能換來第一筆收入。待遇在接下之前都寫得清楚。</p><div class="work-grid">${offers.map((offer) => `<article class="work-card"><h2>${escapeHtml(offer.title)}</h2><p class="work-pay">${offer.pay} 銅</p><dl><div><dt>伙食</dt><dd>${escapeHtml(provisionLabel(offer.provisions.food))}</dd></div><div><dt>飲水</dt><dd>${escapeHtml(provisionLabel(offer.provisions.water))}</dd></div><div><dt>住宿</dt><dd>${escapeHtml(provisionLabel(offer.provisions.lodging))}</dd></div></dl><form method="post" action="/work/accept"><button class="scene-choice" type="submit" name="workId" value="${escapeHtml(offer.id)}">接下這份工作</button></form></article>`).join('')}</div><p class="notice">目前持有 ${copper} 銅 · 已完成 ${historyCount} 份工作。</p><a class="text-link" href="/play">先不工作</a></main><aside class="ad-slot" aria-label="廣告"></aside></div>`);
}
