...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/auth/callback") return finishDiscordLogin(request, url, env);
...
async function finishDiscordLogin(request, url, env) {
  ...
  if (String(user.id) !== String(env.ALLOWED_DISCORD_USER_ID)) {
    return html(messagePage("封閉測試中", "此 Discord 帳號目前沒有《不朽之旅》測試資格。"), 403);
  }
...
function entryPage() {
  return shell(`
    <main class="hero">
      <div class="seal">仙遊者</div>
      <p class="eyebrow">IMMORTALVOYAGE</p>
      <h1>不朽之旅</h1>
      <p class="subtitle">一念入世，一念成仙。你的故事，將由此開始。</p>
      <div class="mist"></div>
      <a class="discord" href="/auth/discord">以 Discord 登入</a>
      <p class="notice">封閉測試 · 僅限指定帳號</p>
    </main>`);
}
...
function shell(content) {
  ...
  *{box-sizing:border-box}html,body{margin:0;min-height:100%;background:#07100f;color:#eee7d5;font-family:"Noto Serif TC","Microsoft JhengHei",serif}body{min-height:100vh;display:grid;place-items:center;overflow:hidden;background:radial-gradient(circle at 50% 20%,#24372f 0,#0d1916 36%,#050a09 76%)}body:before,body:after{content:"";position:fixed;border:1px solid rgba(205,174,102,.18);inset:22px;pointer-events:none}body:after{inset:30px;border-color:rgba(205,174,102,.07)}.hero{position:relative;width:min(92vw,1050px);min-height:min(88vh,760px);padding:72px 40px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}.hero:before{content:"";position:absolute;width:min(65vw,620px);aspect-ratio:1;border-radius:50%;background:radial-gradient(circle,rgba(188,156,83,.09),transparent 68%);filter:blur(8px);z-index:-1}.seal{position:absolute;top:8%;right:8%;writing-mode:vertical-rl;border:1px solid rgba(194,70,54,.5);color:#bd6658;padding:10px 7px;letter-spacing:.2em}.eyebrow{letter-spacing:.5em;font-size:12px;color:#b7a77e;margin:0 0 22px}h1{font-weight:400;font-size:clamp(46px,8vw,92px);letter-spacing:.16em;margin:0;text-shadow:0 8px 40px #000}.subtitle{max-width:620px;font-size:clamp(15px,2vw,20px);line-height:2;color:#c9c2af;margin:28px 0 38px}.discord{display:inline-block;text-decoration:none;color:#f3ead3;border:1px solid rgba(210,184,119,.5);padding:15px 34px;letter-spacing:.14em;background:rgba(13,22,19,.72);backdrop-filter:blur(12px);transition:.2s}.discord:hover{background:rgba(194,164,91,.12);transform:translateY(-2px);box-shadow:0 12px 40px rgba(0,0,0,.28)}.notice{font-size:12px;letter-spacing:.18em;color:#77776d;margin-top:20px}.mist{height:1px;width:min(65vw,480px);background:linear-gradient(90deg,transparent,rgba(209,186,127,.45),transparent);margin-bottom:34px}@media(max-width:600px){body:before{inset:10px}body:after{inset:16px}.hero{width:100%;min-height:100svh;padding:56px 26px}.seal{top:7%;right:8%}h1{font-size:clamp(42px,15vw,64px)}.subtitle{font-size:15px;margin:24px 0 30px}.discord{width:min(82vw,320px);padding:16px 18px}}
