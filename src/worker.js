...
import { getFirstPlayableScene, resolveFirstPlayableAction } from './core/first-playable-scene.js';
...
async function firstSceneAction(request, env) {
  ...
  const form = await request.formData();
  try {
    const outcome = resolveFirstPlayableAction(resolved.character, form.get('actionId'));
    return html(sceneResultPage(resolved.character, outcome));
  } catch (error) {
...
function gamePage(character) {
  ...
  return shell(`<main class="hero play-scene"><p class="eyebrow">IMMORTALVOYAGE</p><p class="scene-name">${name}</p><h1>${escapeHtml(scene.title)}</h1><p class="subtitle">${escapeHtml(scene.body)}</p><form method="post" action="/scene/action" class="scene-actions">${scene.choices.map((choice) => `<button class="scene-choice" type="submit" name="actionId" value="${escapeHtml(choice.id)}">${escapeHtml(choice.label)}</button>`).join('')}</form><p class="notice">這是第一個可玩情境；目前行動只產生局部敘事，不會直接修改共享世界。</p></main>`);
}

function sceneResultPage(character, outcome) {
  const name = escapeHtml(character?.name ?? '無名之人');
  return shell(`<main class="hero play-scene"><p class="eyebrow">IMMORTALVOYAGE</p><p class="scene-name">${name}</p><h1>此行有果</h1><p class="subtitle">${escapeHtml(outcome.result)}</p><a class="button" href="/play">回到眼前</a><p class="notice">下一輪會把情境狀態正式接入存檔與後續事件。</p></main>`);
}
