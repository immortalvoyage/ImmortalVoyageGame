...
function gamePage(character) {
  ...
  return shell(`<main class="hero play-scene"><p class="eyebrow">IMMORTALVOYAGE</p><p class="scene-name">${name}</p><h1>${escapeHtml(scene.title)}</h1><p class="subtitle">${escapeHtml(scene.body)}</p>${persistedResult}<form method="post" action="/scene/action" class="scene-actions">${scene.choices.map((choice) => `<button class="scene-choice" type="submit" name="actionId" value="${escapeHtml(choice.id)}">${escapeHtml(choice.label)}</button>`).join('')}</form><p class="notice">此情境已有 ${historyCount} 筆行動寫入角色存檔；重新登入仍會保留。</p></main>`);
}
