...
- PR #76 candidate: exact-source Estate settlement primitive harness — 6/6 passed, covering absence of player-callable actions, active inventory/money + Trade escrow consolidation, invalid cause, stale character authority, repeated settlement, and overflow zero-mutation behavior; Estate module exact source passed `node --check`. New v5 archive/Estate invariant + additive v4→v5 migration logic harness — 6/6 passed, covering archive/Estate pairing, no duplicated spendable assets, active/archive character-ID exclusion, valid Estate stack quantities, and preservation of active assets. Exact-source world/content compatibility harness — 4/4 passed and validator source passed `node --check`, covering historical archive location retirement plus fail-closed pending-Estate item references. Repository tests add next-life zero-inheritance coverage and assert that no `estate.resolve-death` player action exists.
