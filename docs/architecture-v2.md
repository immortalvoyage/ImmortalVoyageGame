...
World schema v3 adds `behaviorCounts` and migrates v2 saves by backfilling an empty map while preserving any valid existing counts. Adding deterministic NPC interaction sources, familiarity-aware response text, or derived Relationship views does not require another schema change.
